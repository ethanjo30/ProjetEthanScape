...
const BookingPage = () => {
  ...
  useEffect(() => {
    axios.get(`${API}/escapes`)
      .then(res => {
        setEscapes(res.data);
        if (res.data.length > 0) setSelectedEscape(res.data[0]);
      })
  ...
  useEffect(() => {
    let category = "";
    if (players >= 4 && players <= 9) category = "4-9";
    else if (players <= 19) category = "10-19";
    else if (players <= 29) category = "20-29";
    else if (players <= 39) category = "30-39";

    if (category) {
      const pricePerPerson = PRICING[category][duration];
      setTotalPrice(pricePerPerson * players);
    }
  }, [players, duration]);

  return (
    <div className="min-h-screen pt-24 pb-12 px-6 bg-[#020617] text-white">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-4xl font-bold mb-10 text-center text-amber-400 font-serif">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-slate-900/50 p-8 rounded-2xl border border-slate-800 space-y-6">
            <div>
              <label className="text-slate-400 mb-2 flex items-center gap-2 italic">
                <Calendar size={18} /> Scénario
              </label>
              <select 
                className="w-full bg-slate-800 border border-slate-700 p-3 rounded-lg outline-none focus:border-amber-400"
                onChange={(e) => setSelectedEscape(escapes.find(g => g.id === parseInt(e.target.value)))}
              >
                {escapes.map(game => (
            ...
            <div>
              <label className="text-slate-400 mb-4 flex justify-between items-center">
                <span className="flex items-center gap-2"><Users size={18} /> Joueurs</span>
                <span className="text-amber-400 font-bold text-xl">{players}</span>
              </label>
              <input 
                type="range" min="4" max="39" value={players}
            ...
            <div>
              <label className="text-slate-400 mb-4 flex items-center gap-2">
                <Clock size={18} /> Durée (min)
              </label>
              <div className="flex gap-3">
                {[30, 60, 90].map(d => (
                  <button 
                    key={d} onClick={() => setDuration(d)}
                    className={`flex-1 py-3 rounded-lg border font-bold transition-all ${duration === d ? 'bg-amber-400 text-slate-900 border-amber-400' : 'border-slate-700 hover:border-slate-500'}`}
                  >
                    {d}
                  </button>
                ))}
          ...
          <div className="bg-amber-400 text-slate-900 p-8 rounded-2xl flex flex-col justify-between shadow-2xl">
            <div>
              <h2 className="text-2xl font-black mb-6 flex items-center gap-2 uppercase tracking-tighter">
                <CreditCard /> Votre Devis
              </h2>
              <div className="space-y-4 text-lg border-t border-slate-950/10 pt-6">
                <div className="flex justify-between font-medium italic opacity-80">
                  <span>Prix par personne</span>
                  <span>{totalPrice / players} €</span>
                </div>
            ...
            <button className="mt-8 bg-slate-950 text-white py-4 rounded-xl font-bold text-xl hover:scale-105 transition-transform">
              Réserver Maintenant
            </button>
          </div>
        </div>
