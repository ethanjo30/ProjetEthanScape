  ...
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-4">
      ...
      <p className="text-2xl font-bold text-white">
        {stats.escapes?.active || 0}/{stats.escapes?.total || 0}
      </p>
    </div>
    ...
    <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-4">
      ...
      <p className="text-2xl font-bold text-white">{stats.reservations?.total || 0}</p>
      <p className="text-amber-400 text-xs">{stats.reservations?.pending || 0} en attente</p>
    </div>
    ...
    <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-4">
      ...
      <p className="text-2xl font-bold text-white">{stats.contacts || 0}</p>
    </div>
  </div>
) : (
  ...
  {escapes && escapes.length > 0 ? (
    escapes.map((escape) => (
      <tr key={escape.id} className="border-b border-slate-800">
        ...
        <td className="py-3 px-2 text-right">
          <Button variant="ghost" size="sm" onClick={() => openEscapeModal(escape)}>
            <Edit size={16} />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => deleteEscape(escape.id)} className="text-red-400 hover:text-red-300">
            <Trash2 size={16} />
          </Button>
        </td>
      </tr>
    ...
    <tr>
      <td colSpan="7" className="py-4 text-center text-slate-400">
        {loading ? "Chargement en cours..." : "Aucun escape trouvé."}
      </td>
    </tr>
  )}
</tbody>
